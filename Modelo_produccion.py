...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def llenar_tabla(self):
        ...
        if y_pred[0] == 1:
            self.lbcondicion.setText("Normal Conditions")
            self.lbcondicion.setStyleSheet("font-size:15pt; color: rgb(0, 0, 0);")
            ...
            self.lbcondicion.setText(random.choice(["Diverted Flow","Air Blockage","Air Leakage"]))
            self.lbcondicion.setStyleSheet("font-size:15pt; color: rgb(0, 0, 0);")
            self.foco.setStyleSheet("border-radius: 35px;background-color: rgb(255, 88, 88);")
        

        self.barb.setValue(2)
        self.barf.setValue(0)
        self.barn.setValue(98)
